...
# shellcheck disable=SC2034 # SCRIPT_VERSION is exported/used externally at runtime
...
# Initialize log (failure will exit due to -e)
...
# NOTE: Avoid 'cat | xq | jq' to satisfy ShellCheck SC2002/SC2312; parse once then reuse
xml_json="$(xq <"${CONFIG_XML}")"

arr_url_base="$(printf '%s' "${xml_json}" | jq -r .Config.UrlBase)"
...
arr_api_key="$(printf '%s' "$xml_json" | jq -r .Config.ApiKey)"
...
arr_instance_name="$(printf '%s' "$xml_json" | jq -r .Config.InstanceName)"
if [[ $arr_instance_name == "null" || -z $arr_instance_name ]]; then
	arr_instance_name="Lidarr" # fallback
fi

arr_port="$(printf '%s' "$xml_json" | jq -r .Config.Port)"
if [[ -z $arr_port || $arr_port == "null" ]]; then
	case "${arr_instance_name,,}" in
	*sonarr*)
		arr_port="8989"
		log_warning "API port not found, falling back to :8989 (Sonarr default)"
		;;
	*radarr*)
		arr_port="7878"
		log_warning "API port not found, falling back to :7878 (Radarr default)"
		;;
	*)
		arr_port="8686"
		log_warning "API port not found, falling back to :8686 (Lidarr default)"
		;;
	esac
fi
...
arrApiVersion=""
for ver in v3 v1; do
	response="$(curl -s --fail -H "X-Api-Key: $arr_api_key" "${arrUrl}/api/${ver}/system/status" 2>/dev/null || true)"
...
waitForArrApi() {
	...
	for ((i = 1; i <= retries; i++)); do
		if curl -s --fail -H "X-Api-Key: $arrApiKey" "${arrUrl}/api/${arrApiVersion}/system/status" >/dev/null 2>&1; then
			return 0
		fi
		sleep 5
	done
