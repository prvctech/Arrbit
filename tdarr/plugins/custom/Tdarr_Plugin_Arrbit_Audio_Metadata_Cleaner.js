const details = () => ({
  ...
  Type: 'Video',
  Operation: 'Transcode',
  Description: 'This plugin removes title metadata from audio streams, with the ability to ignore specified words.\n\n',
  Version: '2.4',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (inputs.allowed_words && inputs.allowed_words.trim() !== '') {
    allowedWords = inputs.allowed_words.toLowerCase().split(',').map(word => word.trim());
  }
    ...
    if (stream.codec_type.toLowerCase() === 'audio') {
      try {
        ...
        if (title && title !== '""' && title !== '') {
          ...
          const shouldIgnore = allowedWords.some(word => titleLower.includes(word));
