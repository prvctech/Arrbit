...
# Version: v1.2-gs2.6
# Purpose: Overwrite ALL quality definitions in Lidarr with those from JSON, but only if different (Golden Standard v2.6)
# -------------------------------------------------------------------------------------------------------------
...
jq -c '.[]' "$JSON_PATH" | while IFS= read -r definition; do
  ...
  if [[ -n "$match_payload" ]]; then
    # Compare normalized JSON for true 1:1 match (order independent)
    if [[ "$(echo "$payload" | jq -S .)" == "$(echo "$match_payload" | jq -S .)" ]]; then
      log_info "Quality definition matches exactly, skipping: ${title} (ID: ${match_id})"
      printf '[Arrbit] SKIP Quality definition unchanged: %s (Lidarr ID: %s)\n' "$title" "$match_id" | arrbitLogClean >> "$LOG_FILE"
      continue
    fi
