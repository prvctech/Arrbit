...
# Purpose: Configure Lidarr Metadata Write settings via API (Golden Standard v2.7.1 compliant)
# -------------------------------------------------------------------------------------------------------------
...
PAYLOAD_PATH=$(get_yaml_value "autoconfig.paths.metadata_write_payload")
if [[ -z "$PAYLOAD_PATH" || "$PAYLOAD_PATH" == "null" ]]; then
  PAYLOAD_PATH="/config/arrbit/modules/data/payload-metadata_write.json"
fi
...
[FIX]: Place a valid payload-metadata_write.json in $(dirname "$PAYLOAD_PATH") or update the path in configuration:
      autoconfig:
        paths:
          metadata_write_payload: "/path/to/your/payload-metadata_write.json"
EOF
  exit 1
fi
...
# Log to file only, not terminal
printf '[Arrbit] Checking current metadata write settings\n' | arrbitLogClean >> "$LOG_FILE"
current_settings=$(arr_api "${arrUrl}/api/${arrApiVersion}/config/metadataProvider")
