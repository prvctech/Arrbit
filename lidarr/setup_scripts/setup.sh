...
# 2a) Source arrbit.conf so we can read the flags
if [ -f /config/arrbit/config/arrbit.conf ]; then
...
# 4) Download each core module under modules/, including all new ones
...
if [ -x /config/arrbit/setup_scripts/dependencies.bash ]; then
  ...
  bash -c "/config/arrbit/setup_scripts/dependencies.bash" \
...
if [ "${INSTALL_COMMUNITY_PLUGINS:-false}" = "true" ] && [ -x /config/arrbit/process_scripts/plugins_add.bash ]; then
  echo "*** [Arrbit] INSTALL_COMMUNITY_PLUGINS is true – running plugins_add.bash... ***"
  bash -c "/config/arrbit/process_scripts/plugins_add.bash" \
...
if [ "${INSTALL_AUTOCONFIG:-false}" = "true" ] && [ -x /config/arrbit/process_scripts/autoconfig.bash ]; then
  echo "*** [Arrbit] INSTALL_AUTOCONFIG is true – running autoconfig.bash... ***"
  bash -c "/config/arrbit/process_scripts/autoconfig.bash" \
